...
function formatProfessorOptionLabel(professor: LiveUser): string {
  const normalizedEmail = professor.email?.trim() || "No email provided";
  const normalizedName = professor.name?.trim();
...
export function StudentOfficeHours() {
  ...
  const [professors, setProfessors] = useState<OfficeHourContact[]>([]);
  const [availableGroups, setAvailableGroups] = useState<LiveGroup[]>([]);
  ...
  useEffect(() => {
    ...
    const loadOfficeHoursData = async () => {
      ...
      try {
        const [professorsResponse, groupsResponse, requestsResponse] = await Promise.all([
          fetch(`${USERS_API_BASE_URL}?role=professor`),
          fetch(GROUPS_API_BASE_URL),
          fetch(`${OFFICE_HOURS_API_BASE_URL}/student/${user.sub}`),
        ]);
        ...
        const liveGroups = (await groupsResponse.json()) as LiveGroup[];
        const requests = (await requestsResponse.json()) as PastRequest[];

        const contacts = liveProfessors.map((professor) => ({
          id: professor.authId?.trim() || professor.id,
          name: professor.name?.trim() || professor.email?.trim() || "Professor",
          email: professor.email?.trim() || "No email provided",
          department: "Professor",
          displayLabel: formatProfessorOptionLabel(professor),
          lookupIds: [professor.authId?.trim(), professor.id?.trim()].filter((value): value is string => Boolean(value)),
        }));

        if (isMounted) {
          setProfessors(contacts);
          setAvailableGroups(liveGroups);
          setSelectedGroupId((currentGroupId) =>
            currentGroupId && liveGroups.some((group) => group.id === currentGroupId)
              ? currentGroupId
              : "",
          );
  ...
  const filteredProfessors = useMemo(() => {
    if (!selectedGroup?.professor) {
      return professors;
    }

    const matchedProfessors = professors.filter((professor) => professor.lookupIds.includes(selectedGroup.professor ?? ""));
    return matchedProfessors.length > 0 ? matchedProfessors : professors;
  }, [professors, selectedGroup]);
  ...
  useEffect(() => {
    ...
    if (!filteredProfessors.some((professor) => professor.id === selectedProf)) {
      setSelectedProf("");
    }
  }, [filteredProfessors, selectedProf]);

  const canSubmit = Boolean(selectedGroupId && selectedProf && reason.trim() && date && startTime && endTime && meetingType && user?.sub);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(OFFICE_HOURS_API_BASE_URL, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          groupName: selectedGroup.title?.trim() || "Untitled Group",
          professorId: selectedProf,
          professorName: prof?.name ?? selectedProf,
          department: selectedGroup.title?.trim() || prof?.department || "Course",
          reason,
          date,
          startTime,
          endTime,
