...
/** Topic drill-down: with a group slug use professor topic page; without, use student topic analysis route. */
function adminTopicPath(groupName: string | undefined, topicId: string) {
  if (groupName) return `/analysis/${encodeURIComponent(groupName)}/topic/${encodeURIComponent(topicId)}`;
  return `/analysis/topic/${encodeURIComponent(topicId)}`;
}
                    ...
                    >
                      {topicData.map((entry) => (
                        <Cell
                          ...
                          onClick={() => navigate(adminTopicPath(groupName, entry.id))}
                        />
                      ))}
                    </Pie>
                    <Tooltip />
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (t) navigate(adminTopicPath(groupName, t.id));
                        }}
                        style={{
                          background: "none",
                          border: "none",
